with classified as (
  select
    ...
    case when accommodation_type=lower('Whole house or bungalow: Detached') then cast(2 as integer)
      ...
      when accommodation_type=lower('A caravan or other mobile or temporary structure') then cast(4 as integer)
    ...
    case when ethnicity=lower('Asian, Asian British or Asian Welsh') then cast(1 as integer)
      when ethnicity=lower('Black, Black British, Black Welsh, Caribbean or African') then cast(2 as integer)
      when ethnicity=lower('Mixed or Multiple ethnic groups') then cast(3 as integer)
      when ethnicity in (lower('White: English, Welsh, Scottish, Northern Irish or British'), lower('White: Irish')) then cast(0 as integer)
      when ethnicity=lower('White: Gypsy or Irish Traveller, Roma or Other White') then cast(4 as integer)
...
from classified
group by post_town, accommodation_type, accommodation_type_order, ethnicity, ethnicity_order